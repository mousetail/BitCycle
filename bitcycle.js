...
const PLAIN_CHARS = "@! {>\\/|-^=}?\nv+<.~&`()[]#:;,*\"$'%";
const ENCODED_CHARS = Object.freeze(
  "abcdeijlmnopqrstwxz_A_B_C_D_E_F_H_I_J_L_O_Q_S_T_X".match(/_?./g)
);
        ...
        if (device instanceof Collector) {
          ...
          switch (device.chr) {
            ...
            case "~":
              ...
              this.activeBits.push(
                new Bit(bit.x, bit.y, turnLeft(bit.direction), !bit.value)
              );
              bit.direction = turnRight(bit.direction);
              break;
            case "=":
...
function drawBitsAt(bitCode, x, y, scale = 1.0) {
  if (bitCode > 0) {
